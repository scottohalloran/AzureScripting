...
az network vnet subnet create  --name GatewaySubnet --resource-group ContosoResourceGroup  --vnet-name ManufacturingVnet --address-prefixes 10.30.0.0/27
...
#Create vitual machines to test the configuration connections
...
az network nic create --name CoreServicesVM-nic --network-security-group CoreServicesVM-nsg --public-ip-address CoreServicesVM-ip --resource-group ContosoResourceGroup --subnet DatabaseSubnet --vnet-name CoreServicesVnet 
az network nic create --name ManufacturingVM-nic --network-security-group NanufacturingVM-nsg --public-ip-address ManufacturingVM-ip --resource-group ContosoResourceGroup --subnet ManufacturingSystemSubnet --vnet-name ManufacturingVnet --location westeurope

#Create the VMs
az vm create --admin-username TestUser --admin-password 'TestPa$$w0rd!' --image Win2019Datacenter --name CoreServicesVM --public-ip-sku Basic --resource-group ContosoResourceGroup --nics CoreServicesVM-nic --size Standard_DS1_v2
az vm create --admin-username TestUser --admin-password 'TestPa$$w0rd!' --image Win2019Datacenter --name ManufacturingVM --public-ip-sku Basic --resource-group ContosoResourceGroup --nics ManufacturingVM-nic --size Standard_DS1_v2 --location westeurope
